...
class PagoDao @Inject()(protected val dbConfigProvider: DatabaseConfigProvider) extends Tables with HasDatabaseConfigProvider[JdbcProfile] {
  ...
  def findPagosPendientes(implicit ec: ExecutionContext, familiaId: Int) ={
    db.run(getPagos(familiaId).result ) map {
      dataTuples =>
        val pagos = dataTuples.map {pct => PagoApi(pct._1.id, pct._1.recibo, pct._2.valor, pct._2.mes, pct._2.anio, pct._3.getOrElse("").toString,pct._1.familia.toString, pct._1.descuentoAplicado, pct._1.interes, pct._1.pagoParcial, pct._1.resuelto)}
  ...
  private def getPagos(familiaId: Int) = {
   ...
   def getPagosPendientesByIds(implicit ec: ExecutionContext, ids: Seq[Int]) = {
    ...
    db.run(byIdsYAgregoCuotasBaseYTipoPago.result) map { data =>
      data.map{ pct =>
        PagoApi(pct._1.id, pct._1.recibo, pct._2.valor, pct._2.mes, pct._2.anio, pct._3.getOrElse("").toString,pct._1.familia.toString, pct._1.descuentoAplicado, pct._1.interes, pct._1.pagoParcial, pct._1.resuelto)

      }.toList
    }
  }

  def resolver(id:Int) = db.run{
    val q = for { c <- pagos if c.id === id } yield c.resuelto
    q.update(true)
  }
